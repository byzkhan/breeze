...
function showSuggestion(tabId, command, explanation, dangerous) {
  const tab = tabs.get(tabId);
  if (!tab) return;
  // Sanitize: collapse newlines so displayed and executed commands match
  const sanitizedCmd = command.replace(/\n/g, ' ');
  tab.pendingCommand = sanitizedCmd;
  const icon = dangerous ? "\x1b[31m\u26A0\x1b[0m" : "\x1b[32m\u2713\x1b[0m";
  const cmdColor = dangerous ? "\x1b[31m" : "\x1b[36m";
  const lines = [
    `  ${icon} ${cmdColor}${sanitizedCmd}\x1b[0m`,
